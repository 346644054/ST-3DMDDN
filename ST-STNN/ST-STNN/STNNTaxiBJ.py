from keras.layers import (
    ...
    Embedding,GlobalAveragePooling3D,
...
from keras.layers.convolutional import Convolution2D
from keras.layers import Input,merge,Add,Lambda
from keras.layers.normalization import BatchNormalization
...
def squeeze_excite_block2(input, filter, channel, map_height, map_width, data_format='channels_first', ratio=2,
    ...
    x = GlobalAveragePooling3D(data_format="channels_first")(input)
...
def squeeze_excite_block1(input, filter, channel, map_height, map_width, data_format='channels_first', ratio=2, layer_count=0):
    ...
    init = Reshape((filter*channel, map_height, map_width))(input)
    # init3d = GlobalAveragePooling3D(data_format='channels_first')(input)
    ...
    x = Conv3D(filters=32, kernel_size=(1, 1, 1), strides=(1, 1, 1), padding='same',
    ...
    def f2(input):
        ...
        for i in range(nb_layers):
            if i == 0:
                ...
                if i < nb_layers:
                    tmp_in = BatchNormalization(axis=1, scale=False)(tmp_in)
                    tmp_in = Activation("relu")(tmp_in)
                    tmp_in = Conv3D(filters=nb_filter, kernel_size=(len_closeness, 3, 3), strides=(1, 1, 1),
                                 border_mode="same",dilation_rate=(1,i+1,i+1),
                                 kernel_initializer='glorot_uniform', data_format='channels_first')(tmp_in)
            ...
            if i == 0:
                ...
                if i <= nb_layers-1:
                    ...
                    tmp = Conv3D(filters=nb_filter, kernel_size=(len_closeness, 3, 3), strides=(1, 1, 1),
                                 border_mode="same",dilation_rate=(1,2,2),
...
def ST3DNet(c_conf=(6, 2, 16, 8),p_conf=(6,2,16,8), t_conf=(4, 2, 16, 8), external_dim=8, nb_residual_unit=4):
    ...
    if len_closeness > 0:
        ...
        LSTM_out = ConvLSTM2D(filters=8, kernel_size=(3, 3),
        ...
        # Dense Layer
        if True:
            dense_output = mDense2D(LSTM_out, nb_dense_block=0,
                                    nb_layers=8, growth_rate=4,
                                    nb_filter=16, base_filter=3).layer

        # Conv Layer
        x = Convolution2D(nb_filter=8, nb_row=3,
                          nb_col=3,
                          border_mode="same")(dense_output)
        activation = Activation('relu')(x)
        conv2 = Convolution2D(nb_filter=2, nb_row=3,
                              nb_col=3,
        ...
        for conf in [ p_conf]:
            if conf is not None:
                ...
                input = Input(shape=(nb_flow,len_period, map_height, map_width))  # (2,t_c,h,w)
                main_inputs.append(input)
                input = Reshape((nb_flow*len_period, map_height, map_width))(input)
                # DenseLayer
                if True:
                    dense_output = mDense2D(input, nb_dense_block=0,
                                            nb_layers=8,growth_rate=4,
                                            nb_filter=16, base_filter=3).layer
                else:
                    pass
        ...
        # dense_output = dense_conv3D(nb_filter=nb_filter, len_closeness=len_closeness,
        #                             nb_layers=nb_layers)(input)
        #
        # ###meiyou pengzhan
        #
        # # dense_output = squeeze_excite_block1(input=dense_output, filter=32, channel=6, map_height=32,
        # #                                      map_width=32,
        ...
        # channel = 12
        #
        # dense_output1 = squeeze_excite_block2(input=dense_output, filter=filter, channel=channel, map_height=map_height,
        #                                       map_width=map_width,
        #                                       data_format='channel_first', ratio=2, layer_count=0)
        #
        # dense_output2 = squeeze_excite_block1(input=dense_output, filter=filter, channel=channel, map_height=map_height,
        #                                       map_width=map_width,
        #                                       data_format='channel_first', ratio=2, layer_count=0)
        #
        ...
        #
        #
        # dense_output = Reshape((32*12, map_height, map_width))(dense_output)
        #
        # conv2 = Convolution2D(nb_filter=32, nb_row=3,
        #                       nb_col=3,
        #                       border_mode="same")(dense_output)
        #
        # conv2 = Convolution2D(nb_filter=2, nb_row=3,
        ...
        # dense_output = Activation('relu')(conv2)
        #
    ...
    #     nb_layers = 5
    #     # input = Permute((2, 1, 3, 4))(input)
    #     dense_output = dense_conv3D1(nb_filter=nb_filter, len_closeness=len_seq,
    #                                 nb_layers=nb_layers)(input)
    #
    #
    #
    #     ###meiyou pengzhan
    #     dense_output2 = squeeze_excite_block1(input=dense_output, filter=8, channel=4, map_height=map_height,
    #                                          map_width=map_width,
    #                                          data_format='channel_first', ratio=2, layer_count=0)
    #
    #     dense_output1 = squeeze_excite_block2(input=dense_output, filter=8, channel=4, map_height=map_height,
    #                                           map_width=map_width,
    #                                           data_format='channel_first', ratio=2, layer_count=0)
    #
    #
    ...
    #
    #
    #     dense_output = Reshape((8 * 4, map_height, map_width))(dense_output)
    #
    #     conv2 = Convolution2D(nb_filter=2, nb_row=3,
    #                           nb_col=3,
    #                           border_mode="same")(dense_output)
