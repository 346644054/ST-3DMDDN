# from xiaorong1 import *
from STNNTaxiBJ import *
from utils import *
import argparse
import configparser
import os
...
for p in cs:
    ...
    model.summary()
    plot_model(model, to_file=os.path.join(expdir,'STNNTaxiBJ111111.png'), show_shapes=True)
    ...
    score = model.evaluate(X_test, Y_test, batch_size=Y_test.shape[0]// 20, verbose=0)
    print('Test score: %.6f  rmse (real): %.6f' %(score[0], score[1]))

    Y_test_predict = model.predict(X_test, batch_size=Y_test.shape[0]//20, verbose=0)
