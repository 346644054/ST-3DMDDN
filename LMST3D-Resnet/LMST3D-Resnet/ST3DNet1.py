...
def ST3DNet(c_conf=(6, 2, 16, 8), p_conf=(6,2,16,8), t_conf=(4, 2, 16, 8),
            external_dim=8, mask = (4,2,16,8), nb_residual_unit=4):
    ...
    if len_closeness > 0:
        ...
        # Conv1 3D
        conv = Conv3D(filters=32, kernel_size=(2, 3, 3), strides=(1, 1, 1), border_mode="same",
                      kernel_initializer='random_uniform')(input)

        conv = Conv3D(filters=64, kernel_size=(2, 3, 3), strides=(1, 1, 1), border_mode="same",
                      kernel_initializer='random_uniform')(conv)

        #
        conv1 = Conv3D(filters=64, kernel_size=(2,  3, 3), strides=(1, 1, 1), padding="same",
                        kernel_initializer='random_uniform')(conv)


        conv = keras.layers.Add()([conv1,conv])

        conv4 = Conv3D(filters=64, kernel_size=(2, 3, 3), strides=(1, 1, 1), padding="same",
                       kernel_initializer='random_uniform')(conv)

        conv4 = MaxPooling3D(pool_size=(1, 2, 2))(conv4)
    ...
    if p_conf is not None:
        ...
        main_inputs.append(input)
        conv = Conv3D(filters=32, kernel_size=(3, 3, 3), strides=(1, 1, 1), border_mode="same",
                      kernel_initializer='random_uniform')(input)

        conv = Conv3D(filters=64, kernel_size=(3, 3, 3), strides=(1, 1, 1), border_mode="same",
                      kernel_initializer='random_uniform')(conv)
