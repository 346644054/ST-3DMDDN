...
def train():
    ...
    optimizer = optim.Adam(model.parameters(), lr=dconf.learning_rate)
    model.train()
    for epoch in range(dconf.epochs):
        for i, (X, X_ext, Y, Y_ext) in enumerate(train_loader, 0):

            step = (epoch+1) * len(train_loader)//32 + i
            ...
            # nn.utils.clip_grad_norm_(model.parameters(), dconf.grad_threshold)
            optimizer.step()
