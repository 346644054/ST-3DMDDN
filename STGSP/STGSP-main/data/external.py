...
def external_taxibj(datapath, fourty_eight, previous_meteorol):
    def f(tsx, tsy, ext_time):
        exd = ExtDat(datapath)
        tsx = np.asarray([exd.get_bjextarray(N, ext_time, fourty_eight=fourty_eight) for N in tsx])
        tsy = exd.get_bjextarray(tsy, ext_time, fourty_eight=fourty_eight, previous_meteorol=previous_meteorol)
